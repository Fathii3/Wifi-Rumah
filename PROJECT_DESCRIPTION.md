...
4. **Kustomisasi & Arsitektur Hybrid Berkinerja Tinggi:** Desain visual *Glassmorphic* yang fleksibel, responsif di semua perangkat (Mobile & Desktop), serta dapat dijalankan secara hybrid baik di server lokal (Node.js/Termux) maupun cloud (Vercel Serverless Functions & Edge Config).
...
### 2. Short Summary
**Wi-Fi Connect** is a modern guest portal utility and local network monitoring system featuring a sleek *Glassmorphic* user interface. It enables guests to seamlessly connect to home Wi-Fi via dynamic QR code scanning or one-click password copying, while offering real-time active device tracking and network speed diagnostics for home owners.
...
4. **High-Performance & Hybrid Architecture:** Lightweight, responsive *Glassmorphism* design optimized for all devices, seamlessly supported by both local runtime environments (Node.js/Termux) and cloud deployments (Vercel Serverless Functions & Edge Config).
...
* **Solution:** 
  Engineered a responsive single-page web app using Vanilla JS & Tailwind CSS with modern *Glassmorphic* aesthetics. Developed a Node.js/Express backend integrated with *Puppeteer Headless Automation* to scrape router state. Secured backend communications via an automated *Ngrok HTTPS tunnel* coupled with a *Vercel Serverless Proxy* and *Vercel Edge Config* to shield internal API endpoints from browser inspection.
* **Result:** 
